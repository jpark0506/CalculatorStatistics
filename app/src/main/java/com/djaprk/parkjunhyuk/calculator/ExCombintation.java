...
public class ExCombintation extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ex_combintation);
    ...
    private float caculate(String alpha, String beta) {
        ...
        float ref = 1;
        float temp=1;
        if (um3 < um1 || um1 < 0 || um3 < 0 || um2 / 1000 > 4) {
            Toast.makeText(getApplicationContext(), "숫자를 올바르게 치세요!", Toast.LENGTH_LONG).show();
            result = -2;
        } else if(um1==0){
            result =0;
        }
        else {
            for (float i = um3; i >= um3 - um1 + 1; i--) {
                ref = ref * i;
                if (ref < 0) break;
